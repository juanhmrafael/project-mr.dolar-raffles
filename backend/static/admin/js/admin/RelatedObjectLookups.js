...
'use strict';
{
    ...
    // Opción 1: Usar visibilitychange (más confiable)
    document.addEventListener('visibilitychange', function () {
        if (document.hidden) {
            try {
                window.dismissChildPopups();
            } catch (e) {
                console.log('Error al cerrar popups:', e);
            }
        }
    });

    // Opción 2: Usar pagehide como respaldo
    window.addEventListener('pagehide', function (event) {
        try {
            window.dismissChildPopups();
        } catch (e) {
            console.log('Error al cerrar popups en pagehide:', e);
        }
    });

    // Opción 3: Interceptar navegación
    if ('addEventListener' in window) {
        let isNavigating = false;

        // Detectar cuando se está navegando
